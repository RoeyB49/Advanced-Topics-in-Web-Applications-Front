    ...
    const loadGoogleSignInButton = () => {
      if (window.google && window.google.accounts) {
        window.google.accounts.id.initialize({
          client_id:
            "323396859646-f21h2qhltd3ni9ssc9snstsiusuvlouc.apps.googleusercontent.com",
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        setFormErrors({
          ...formErrors,
          avatar: "Error uploading image. Please try again.",
        });
      } finally {
    ...
    >
      ...
      <Box
        sx={{
          ...
          backgroundColor: "#4a752c", // Changed to a more earthy color
          padding: "15px",
          borderRadius: "8px",
          color: "white",
      ...
      >
        <Typography variant="h6" gutterBottom>
          Why Join LostAndFound?
        </Typography>
        <Typography variant="body2">
          Become part of a community dedicated to helping each other recover
          lost items. By registering, you can post details about lost or found
          items, communicate securely with other users, and get alerts when a
          potential match is found.
        </Typography>
        <Typography variant="subtitle1" gutterBottom sx={{ marginTop: "10px" }}>
          Join now and help us make a difference one item at a time!
        </Typography>
      </Box>
    </Box>
  );
